...
export default function DashboardPage() {
  const { data: guestsData } = useQuery({
    queryKey: qk.guests.list({ limit: 100 }),
    queryFn: () => fetch("/api/guests?limit=100").then((r) => r.json()).then((d) => (d.data ?? []) as Guest[]),
  });
  const { data: bookingsData } = useQuery({
    queryKey: qk.bookings.list(),
    queryFn: () => fetch("/api/bookings").then((r) => r.json()).then((d) => (d.data ?? []) as Booking[]),
  });
