...
export default async function HotelHomePage() {
  ...
  if (guestToken) {
    const payload = await verifyToken(guestToken);
    isGuestLoggedIn = payload?.scope === "guest";
  }
  const guestDest = isGuestLoggedIn ? "/guest/bookings" : "/guest/login";
    ...
    <div className="green-theme" style={{ background: "var(--g-bg)", color: "var(--g-text)", fontFamily: "Inter, system-ui, sans-serif", minHeight: "100vh" }}>
      ...
      <header style={{
        ...
        <nav style={{ display: "flex", alignItems: "center", gap: 24 }}>
          <a href="#rooms" className="g-header-nav-link">Номера</a>
          <a href="#amenities" className="g-header-nav-link">Услуги</a>
          <a href="#contacts" className="g-header-nav-link">Контакты</a>
          <Link href={guestDest} style={{
            ...
            <div key={room.id} className="g-card" style={{ overflow: "hidden", transition: "box-shadow .2s" }}>
              ...
              <div style={{ padding: 20 }}>
                ...
                <Link href={guestDest} style={{
                  ...
                  {isGuestLoggedIn ? "Забронировать" : "Войти и забронировать"}
                </Link>
              </div>
            </div>
          ))}
      ...
      <section style={{
        ...
        <Link href={guestDest} style={{
          ...
          {isGuestLoggedIn ? "Мои бронирования" : "Войти в личный кабинет"}
        </Link>
      </section>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 32, marginBottom: 40 }}>
            ...
            <div>
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: 0, fontSize: 13, lineHeight: 2 }}>
                ...
                <li>
                  <Link href={guestDest} style={{ color: "var(--g-primary)", textDecoration: "none" }}>
                    Личный кабинет
                  </Link>
                </li>
              </ul>
