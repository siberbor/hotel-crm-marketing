...
async function main() {
  ...
  // Clear existing data
  await db.execute(
    sql`TRUNCATE TABLE shifts, tasks, guest_accounts, sync_logs, campaigns, interactions, bookings, guests, rooms, users, permissions RESTART IDENTITY CASCADE`,
  );
  ...
  const tasksData = await db
    ...
    .values([
      {
        title: "Уборка номера 101",
        status: "todo",
        assignedTo: users[3].id, // receptionist
        roomId: rooms[0].id,
      ...
      {
        title: "Подготовить номер 201 к заезду",
        status: "in_progress",
        assignedTo: users[3].id,
        roomId: rooms[2].id,
        scheduledTime: new Date(Date.now() + 60 * 60 * 1000),
        notes: "VIP гость, цветы на стол",
        createdBy: users[1].id,
      },
      {
        title: "Доставить ужин в номер 301",
        status: "urgent",
        assignedTo: users[3].id,
        roomId: rooms[4].id,
        scheduledTime: new Date(Date.now() + 30 * 60 * 1000),
        notes: "Ведёрко со льдом",
        createdBy: users[1].id,
      },
      {
        title: "Проверить мини-бар в номере 102",
        status: "done",
        assignedTo: users[3].id,
        roomId: rooms[1].id,
        scheduledTime: null,
      ...
      {
        title: "Замена полотенец во всех номерах",
        status: "postponed",
        assignedTo: null,
        roomId: null,
  ...
  // Sprint 5: GuestAccounts (для тестирования ЛК гостя)
  const guestPassword = await bcrypt.hash("guest123", saltRounds);
  const guestAccountsData = await db
    .insert(schema.guestAccounts)
    .values([
      {
        guestId: guests[0].id, // Иван Петров
        email: "ivan@test.com",
        passwordHash: guestPassword,
      },
      {
        guestId: guests[4].id, // Дмитрий Морозов (VIP)
        email: "dmitry@test.com",
        passwordHash: guestPassword,
      },
    ])
    .returning();
  console.log(`✅ Created ${guestAccountsData.length} guest accounts`);
  ...
  console.log("\nGuest LK (Personal account):");
  console.log("  ivan@test.com / guest123  (Иван Петров)");
  console.log("  dmitry@test.com / guest123  (Дмитрий Морозов, VIP)");
