...
const EMPTY_FORM = { guestId: "", type: "call" as Interaction["type"], subject: "", content: "" };
...
export default function InteractionsPage() {
  ...
  const { data: interactions = [], isLoading: loading } = useQuery({
    queryKey: qk.interactions.list(),
    queryFn: () => fetch("/api/interactions").then((r) => r.json()).then((d) => (d.data ?? []) as Interaction[]),
    enabled: !!user,
  });

  const { data: guests = [] } = useQuery({
    queryKey: qk.guests.list(),
    queryFn: () => fetch("/api/guests").then((r) => r.json()).then((d) => (d.data ?? []) as Guest[]),
    enabled: !!user,
    staleTime: 5 * 60 * 1000,
  });

  const saveMutation = useMutation({
    mutationFn: () => fetch("/api/interactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: qk.interactions.all }); setShowModal(false); setFormData(EMPTY_FORM); },
  });

  const filteredInteractions = filterType ? interactions.filter((i) => i.type === filterType) : interactions;
        ...
        footer={
          <>
            ...
            <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}>
              {saveMutation.isPending ? "Сохранение..." : "Добавить"}
            </Button>
          </>
        }
      >
